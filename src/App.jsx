...
const isStarWarsTitle = item => /star wars|mandalorian|return of the jedi|empire strikes back|phantom menace|attack of the clones|revenge of the sith|rogue one|\bsolo\b/i.test(item.title || item.name || '')
const disneySkyDots = [{x:'7%',y:'13%',s:2,d:'0s'},{x:'13%',y:'31%',s:3,d:'1.4s',t:true},{x:'19%',y:'19%',s:2,d:'3s'},{x:'24%',y:'48%',s:2,d:'4.5s'},{x:'31%',y:'11%',s:3,d:'2.1s',t:true},{x:'37%',y:'28%',s:2,d:'5s'},{x:'42%',y:'42%',s:3,d:'1s'},{x:'48%',y:'16%',s:2,d:'3.7s'},{x:'54%',y:'30%',s:2,d:'2.7s'},{x:'58%',y:'9%',s:3,d:'5.4s',t:true},{x:'65%',y:'40%',s:2,d:'1.6s'},{x:'70%',y:'19%',s:2,d:'4.2s'},{x:'74%',y:'49%',s:3,d:'3.3s',t:true},{x:'79%',y:'11%',s:2,d:'0.8s'},{x:'92%',y:'37%',s:2,d:'4.8s'},{x:'96%',y:'20%',s:3,d:'2.5s',t:true},{x:'4%',y:'62%',s:2,d:'3.6s'},{x:'17%',y:'67%',s:2,d:'5.8s'},{x:'28%',y:'58%',s:3,d:'1.8s',t:true},{x:'46%',y:'62%',s:2,d:'4.4s'},{x:'61%',y:'58%',s:2,d:'2.9s'},{x:'72%',y:'68%',s:2,d:'0.5s'},{x:'94%',y:'62%',s:2,d:'3.1s'},{x:'10%',y:'43%',s:2,d:'1.9s'},{x:'22%',y:'8%',s:2,d:'3.2s'},{x:'34%',y:'37%',s:2,d:'5.1s'},{x:'39%',y:'57%',s:2,d:'2.3s',t:true},{x:'51%',y:'7%',s:2,d:'4.7s'},{x:'56%',y:'51%',s:2,d:'1.2s'},{x:'68%',y:'28%',s:2,d:'3.9s'},{x:'77%',y:'37%',s:2,d:'0.6s',t:true},{x:'83%',y:'56%',s:2,d:'5.5s'},{x:'89%',y:'8%',s:2,d:'2.8s'},{x:'98%',y:'48%',s:2,d:'4.1s'}]
const pixieDust = [['49%','20%','-11px','0s'],['53%','14%','8px','.6s'],['57%','23%','-6px','1.3s'],['60%','10%','13px','.25s'],['63%','19%','-14px','1.8s'],['66%','7%','7px','.9s'],['69%','16%','-8px','2.2s'],['72%','11%','12px','.45s'],['75%','22%','-5px','1.5s'],['78%','15%','9px','2.6s'],['81%','25%','-13px','1.1s'],['84%','18%','5px','2s'],['55%','30%','-9px','2.8s'],['62%','28%','11px','.75s'],['70%','31%','-12px','1.65s'],['76%','29%','6px','3.1s'],['87%','12%','-7px','1.25s'],['90%','23%','10px','2.35s']]
...
function PosterRow({items,openTitle}){
 ...
 const scrollRow=direction=>rowRef.current?.scrollBy({left:direction*Math.max(rowRef.current.clientWidth*.8,300),behavior:'smooth'})
 return <div className="poster-row-shell"><button className="poster-scroll-control poster-scroll-control--previous" type="button" aria-label="Scroll row left" onClick={()=>scrollRow(-1)}>‹</button><div className="poster-row" ref={rowRef}>{items.map((item,index)=>{const name=typeof item==='string'?item:item.title;const image=typeof item==='string'?art[index]:item.poster||item.image;return <button className="wide-card" key={name} onClick={()=>typeof item!=='string'&&openTitle(item)}><img src={image} alt={`${name} poster`}/><span>{name}</span></button>})}</div><button className="poster-scroll-control poster-scroll-control--next" type="button" aria-label="Scroll row right" onClick={()=>scrollRow(1)}>›</button></div>
}
 ...
 useEffect(()=>{
  ...
  Promise.all(paths.map(path=>fetch(`/api/tmdb?path=${encodeURIComponent(path)}`).then(response=>response.json())))
   .then(([trending,movies,series,marvel,pixar,starwars,natgeo,disney])=>setLiveRows([
    ...
    ['Star Wars saga',starwars.results.filter(item=>!excludedTitles.has(item.title||item.name)&&isStarWarsTitle(item)).slice(0,8).map(toTitle)],
...
function BrandLanding({brand,feature,openTitle}){
 ...
 return <section ref={landingRef} className={`brand-landing ${brand.slug}`}><div className="studio-intro" aria-hidden="true"><span/><i ref={pixarLightRef}/><b/></div>{brand.slug==='disney'&&<div className="disney-sky-stars" aria-hidden="true"><div className="disney-star-field">{disneySkyDots.map((star,index)=><span key={index} className={`disney-sky-dot${star.t?' twinkle':''}`} style={{'--x':star.x,'--y':star.y,'--size':`${star.s}px`,'--delay':star.d}}/>)}</div><span className="disney-shooting-star"/><div className="disney-pixie-trail">{pixieDust.map(([x,y,drift,delay],index)=><b className="disney-comet-dust" key={index} style={{'--dust-x':x,'--dust-y':y,'--dust-drift':drift,'--dust-delay':delay}}/>)}</div><i/><span className="disney-small-star"/></div>}<div className="brand-landing-copy">{brand.slug==='marvel'?<div className="marvel-landing-logo"><b>MARVEL</b><span>STUDIOS</span></div>:<img src={brand.logo} alt={`${brand.name} logo`}/>}<p>{brand.tagline}</p>{feature&&<><p className="hero-meta">{feature.year} · Featured collection</p><button className="watch" onClick={()=>openTitle(feature)}>▶ WATCH FEATURED</button></>}</div></section>
}
