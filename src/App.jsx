...
function BrandLanding({brand,feature,openTitle}){
 ...
 return <section ref={landingRef} className={`brand-landing ${brand.slug}`}><div className="studio-intro" aria-hidden="true"><span/><i ref={pixarLightRef}/><b/></div>{brand.slug==='disney'&&<div className="disney-sky-stars" aria-hidden="true"><div className="disney-star-field">{disneySkyDots.map((star,index)=><span key={index} className={`disney-sky-dot${star.t?' twinkle':''}`} style={{'--x':star.x,'--y':star.y,'--size':`${star.s}px`,'--delay':star.d}}/>)}</div><span className="disney-shooting-star">{Array.from({length:9},(_,index)=><b className="disney-comet-dust" key={index}/>)}</span><i/><span className="disney-small-star"/></div>}<div className="brand-landing-copy">{brand.slug==='marvel'?<div className="marvel-landing-logo"><b>MARVEL</b><span>STUDIOS</span></div>:<img src={brand.logo} alt={`${brand.name} logo`}/>}<p>{brand.tagline}</p>{feature&&<><p className="hero-meta">{feature.year} · Featured collection</p><button className="watch" onClick={()=>openTitle(feature)}>▶ WATCH FEATURED</button></>}</div></section>
}
