...
function HeroMedia({titles,openTitle,toggleList}){
 ...
 return <section className="plus-hero" id="home" style={trailer?undefined:{backgroundImage:`linear-gradient(90deg,#040714 10%,rgba(4,7,20,.67) 44%,rgba(4,7,20,.05)),url(${title?.image||art[0]})`}}>{trailer&&<iframe ref={trailerFrame} className="hero-trailer" title={`${title.title} trailer`} src={trailerUrl} allow="autoplay; encrypted-media"/>}<div className="hero-shade"/><div className="plus-hero-copy"><p className="eyebrow">DISNEY+ FEATURED</p>{logo?<img className="hero-title-logo" src={`https://image.tmdb.org/t/p/w500${logo.file_path}`} alt={title.title}/>:<p className="hero-title-fallback">{title?.title||'Moana'}</p>}<p className="hero-meta">{title?.year||'2024'} · Family · Adventure</p><p>{title?.overview||'Moana answers the call of the ocean and sets sail with Maui on an unforgettable journey across the seas of Oceania.'}</p><button className="watch" onClick={()=>openTitle(title)}>▶ WATCH NOW</button>{trailer&&<button className="sound-toggle" onClick={startSound}>{soundOn?'🔊 RESTART SOUND':'🔇 TURN SOUND ON'}</button>}<button className="add" onClick={()=>toggleList(title)} aria-label="Add featured title to My List">＋</button></div>{titles.length>1&&<div className="hero-carousel" aria-label="Featured trailers"><button onClick={()=>move(-1)} aria-label="Previous trailer">‹</button><div>{titles.map((item,index)=><button className={index===slide?'active':''} key={item.id} onClick={()=>setSlide(index)} aria-label={`Show ${item.title}`}/>)}</div><button onClick={()=>move(1)} aria-label="Next trailer">›</button></div>}</section>
}
...
export default function App(){
 ...
 const visibleRows=page==='brand'?catalogRows.filter(([title])=>title===brandRows[brand]):page==='movies'?catalogRows.filter(([title])=>['New to Disney+','Marvel Studios','Pixar favorites','Star Wars saga','Disney stories'].includes(title)):page==='series'?catalogRows.filter(([title])=>['Binge-worthy series','National Geographic','Recommended for you'].includes(title)):page==='originals'?catalogRows.filter(([title])=>['Recommended for you','Disney stories','National Geographic'].includes(title)):catalogRows
